...
No painel web, a aba Env Vault lista apenas metadados por padrão. Para consulta pontual, clique em `Desbloquear` e escolha `Segredo do Env Vault`, `Recovery key` ou o envelope de conta quando disponível. Se o env ainda não tiver recovery, informe o segredo atual uma vez em `Configurar recovery key`; o navegador valida o env, gera uma recovery key e envia ao backend somente um envelope cifrado. A descriptografia acontece no navegador; segredo, recovery key e plaintext não são enviados ao backend. Para restaurar o arquivo no notebook, use `myinst env pull --output ...` na pasta do projeto.
